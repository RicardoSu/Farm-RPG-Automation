...
class Vision:
    ...
    # constructor
    def __init__(self, fish_img_path, method=cv.TM_CCOEFF_NORMED):
        # load the image we're trying to match
        # https://docs.opencv.org/4.2.0/d4/da8/group__imgcodecs.html
        self.fish_img = cv.imread(fish_img_path, cv.IMREAD_UNCHANGED)

        # Save the dimensions of the needle image
        self.fish_img_width = self.fish_img.shape[1]
        self.fish_img_height = self.fish_img.shape[0]
    ...
    def find(self, haystack_img, threshold=0.5, max_results=20):
        ...
        result = cv.matchTemplate(haystack_img, self.fish_img, self.method)
        ...
        # We can eliminate those redundant
        # locations by using groupRectangles().
        # First we need to create the list of [x, y, w, h] rectangles

        rectangles = []
        for loc in locations:
            rect = [int(loc[0]), int(loc[1]), self.fish_img_width, self.fish_img_height]
    ...
    def match_keypoints(self, original_image, patch_size=32):
        ...
        keypoints_needle, descriptors_needle = orb.detectAndCompute(self.fish_img, None)
